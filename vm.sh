...
validate_input() {
    ...
    case $type in
        ...
        "port")
            ...
            if [ "$value" -lt 23 ] || [ "$value" -gt 65535 ]; then
                print_status "ERROR" "Must be a valid port number (23-65535)"
                return 1
            fi
            ;;
...
check_dependencies() {
    local deps=("qemu-system-x86_64" "wget" "cloud-localds" "qemu-img" "ss" "openssl" "pgrep" "pkill")
    local missing_deps=()
    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        ...
        print_status "INFO" "On Ubuntu/Debian, try: sudo apt update && sudo apt install qemu-system-x86 cloud-image-utils wget iproute2 openssl procps"
        exit 1
    fi
}
...
cleanup() {
    # remove temp cloud-init files in VM_DIR if present
    if [ -n "${VM_DIR:-}" ]; then
        rm -f "$VM_DIR"/user-data "$VM_DIR"/meta-data 2>/dev/null || true
    fi
}
...
load_vm_config() {
    ...
    if [[ -f "$config_file" ]]; then
        # Clear previous variables (only ones we expect)
...
create_new_vm() {
    ...
    while true; do
        ...
        if validate_input "port" "$SSH_PORT"; then
            ...
            if ss -ltn 2>/dev/null | awk '{print $4}' | grep -qE "(:|\\b)$SSH_PORT\$"; then
...
setup_vm_image() {
    ...
    # Try resizing the disk image if needed
    if ! qemu-img resize "$IMG_FILE" "$DISK_SIZE" 2>/dev/null; then
        print_status "WARN" "qemu-img resize failed (maybe image is not writable). Creating a new qcow2 overlay with requested size..."
        local base_img="$IMG_FILE"
        local new_img="$IMG_FILE.new"
        # create overlay with backing file
        if qemu-img create -f qcow2 -b "$base_img" "$new_img" "$DISK_SIZE"; then
            mv -f "$new_img" "$IMG_FILE"
            print_status "INFO" "Created overlay image with backing file."
        else
            print_status "WARN" "Failed to create overlay backed image. Creating empty qcow2 image instead..."
            rm -f "$IMG_FILE"
            if qemu-img create -f qcow2 "$IMG_FILE" "$DISK_SIZE"; then
                print_status "INFO" "Created new empty qcow2 image: $IMG_FILE"
            else
    ...
    # cloud-init configuration (created inside VM_DIR)
...
EOF
    ...
    if ! cloud-localds "$SEED_FILE" "$VM_DIR/user-data" "$VM_DIR/meta-data"; then
...
start_vm() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        local kvm_flag=()
        if [ -e /dev/kvm ]; then
            # ensure qemu supports enable-kvm
        ...
        if [[ -n "${PORT_FORWARDS:-}" ]]; then
            ...
            for forward in "${forwards[@]}"; do
                ...
                # create unique ids
                qemu_cmd+=(-device "virtio-net-pci,netdev=net${idx}")
                qemu_cmd+=(-netdev "user,id=net${idx},hostfwd=tcp::$host_port-:$guest_port")
                idx=$((idx+1))
            done
        fi
...
is_vm_running() {
    local vm_name=$1
    # Check for qemu process that has either the image file or instance id in args
    if pgrep -f "qemu-system-x86_64.*$VM_DIR/.*${vm_name}" >/dev/null || pgrep -f "qemu-system-x86_64.*$IMG_FILE" >/dev/null; then
        return 0
    else
        return 1
    fi
                ...
                4)
                    while true; do
                        ...
                        if validate_input "port" "$new_ssh_port"; then
                            # Check if port is already in use
                            if [ "$new_ssh_port" != "$SSH_PORT" ] && ss -ltn 2>/dev/null | awk '{print $4}' | grep -qE "(:|\\b)$new_ssh_port\$"; then
...
show_vm_performance() {
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            ...
            # Get QEMU process ID
            local qemu_pid
            qemu_pid=$(pgrep -f "qemu-system-x86_64.*$IMG_FILE" | head -n1 || true)
            if [[ -n "$qemu_pid" ]]; then
                # Show process stats
...
# Supported OS list (format: os_name="os_type|codename|img_url|default_hostname|default_username|default_password")
