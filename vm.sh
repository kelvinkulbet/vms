...
# Check if virtualization is enabled in CPU
...
if ! check_kvm; then
    print_status "ERROR" "KVM is not available. Please fix the issues above and try again."
    exit 1
fi
}
...
local current_size=$(qemu-img info "$IMG_FILE" | grep "virtual size" | awk '{print $3}' | tr -d ',')
...
is_vm_running() {
local vm_name=$1
local vm_config_file="$VM_DIR/$vm_name.conf"

if [[ -f "$vm_config_file" ]]; then
    source "$vm_config_file"
    # Check if QEMU process with this VM's image file is running
